...
load.MSigDB <- function(species){
	...
	library(msigdbr)
	msig_df <- msigdbr(species = species)
	cat(paste0("Printing available geneset categories for: ", species))
	print(msig_df %>% dplyr::distinct(gs_cat, gs_subcat) %>% dplyr::arrange(gs_cat, gs_subcat))
	return(msig_df)
}
...
run.FGSEA <- function(rnk, genesets, nproc = 2, minGenes = 3, maxGenes = 5000, reformat = T, filename = F, minP = 1e-20){
	...
	library(data.table)
	library(dplyr)
	gsea_res <- fgsea(stats = rnk, pathways = genesets, minSize = minGenes, maxSize = maxGenes, eps = minP, nproc = nproc)
	## order results based on NES
	ord_gsea_res <- gsea_res %>% arrange(desc(NES))
