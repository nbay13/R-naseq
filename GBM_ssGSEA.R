load.genesets <- function(wd){
	cur_dir <- getwd()
	setwd(wd)
	genesets <- as.list(read.table("GBM Master ssGSEA genesets.txt", sep = "\t", stringsAsFactors = F, header = T))
...
run.ssGSEA <- function(exp_mat, gene_list, method = "ssgsea", norm = F){
	library(GSVA)
	return(gsva(exp_mat, gene_list, method = method, ssgsea.norm = norm))
}
...
load.genesets.old <- function(){
	...
	neftel_df <- read.table("Filtered_Neftel_2019_cellular_states.txt", sep = "\t", stringsAsFactors = F, header = T)
	...
	superset <- c(neftel_genesets, subtype_genesets, filbin_genesets, venteicher_genesets, patel_genesets, liau_genesets, suva_geneset, 
		zhang_genesets, cahoy_genesets, dusart_hundred, dar_list, bhaduri_list, GO_genesets, GO_genesets_v2, lipid_genesets, SREBF_genesets, splt, splt_v2, psplt, psplt_v2, mcp_genesets, davoli_genesets, estimate_genesets, 
		type_signatures, type_list, garofano_list)
